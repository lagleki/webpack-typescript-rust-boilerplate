...
- check if it's a lojban phrase 100% - show def of each word in same language with fallback to jbo and en
  - if lujvo - decomp it and show veljvocmi in place
  - search in muplis by similar words
- if such word is only in jbo - show it

- if not lojban
  - if a single word find it as normally by substring
...
- does coi.js reload to coop?
  ...
  - cross origin isolated is a must
    - implement reload
- too many .wasm onnx files cached
- add rimni, regex, multiword search
  - take muplis into account
- cache inferred vectors
- ? cant even type new text while loading
  - and after loaded never change the input box automatically unless from url
- selmaho search is not sorted, should clearly have show all selmaho mode without anything else
...
- tcini.json is used to check which files from sutysisku need to be updated
  ...
  - plus additional external links necessary
...
- move emb dict loading to
  - if not in cache then delay to cnino_sorcu
    - and use a dummy function to return results
  - if in cache just load it
  - on update cache the old one must be removed
- when semsearcher is booted then do ninsisku again
- search too slow because page size disabled?
- get distances in semsearcher
- get cosine distance in rust tokenizer
- in searchResult back add embeddings score. from similarDefs
