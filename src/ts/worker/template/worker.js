...
let db, sqlite3, sql, wordEmbeddings
...
function prepareWrapper() {
  ...
  async function execute(sql, bindings) {
    const results = []
    const { sqlite3, db } = self
    for await (const stmt of sqlite3.statements(db, sql)) {
      sqlite3.reset(stmt)
      if (bindings) sqlite3.bind_collection(stmt, bindings, sql)
      const rows = []
...
async function sisku(searching) {
  ...
  if (!leijufra_incoming.bangu || !leijufra.bangu) {
    ...
    if (bangu) {
      ...
      try {
        const res = await sql(`SELECT jufra FROM tejufra where bangu=?`, ['en'])
        tef1 = JSON.parse(res?.[0]?.jufra)
      } catch (error) {}
    }
